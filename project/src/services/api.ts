...
async function apiRequest<T>(endpoint: string, method: string = 'GET', body?: any): Promise<T | null> {
  ...
  // Handle 204 No Content by returning null
...
export const rentAPI = {
  ...
  delete: async (id: string): Promise<boolean> => {
    await apiRequest<void>(`/payments/${id}`, 'DELETE');
    return true;
  },
};
